...
df.columns = ['MCS_unigram(p)','ACS_bigram_tfidf(p)','ARL(p)','AFPP(p)','ASPP(p)','AFTAPP(p)','ASAPP(p)','ASW(p)','AOW(p)','AInW(p)','AImW(p)']
df.to_excel("Data\\YelpZip\\Resulting Features\\Product_Centric_Textual_Features.xlsx", index = False)			# Change the path according to the file location
