...
if __name__ == "__main__":
	...
	#print(X.head())
	#print(X.shape)
	Y= pd.read_excel('Data\\YelpNYC\\Labels_for_products.xlsx', sheetname=0)			# Change the path according to the file location
	Y = Y.iloc[:, 0:1]
	...
	for i in range(0,3):
		...
		filename = 'Data\\Results\\YelpNYC\\Product Behavioral\\'+cls[i]+'\\FPR_TPR_Threshold_Product_Behavioral.xlsx'		# File for Writing FPR and TPR values at different thresholds for ROC curve analysis
		filename1 = 'Data\\Results\\YelpNYC\\Product Behavioral\\'+cls[i]+'\\Pre_Recall_Threshold_Product_Behavioral.xlsx'	# File for Writing Precision and Recall values at different thresholds for Precision-recall curve analysis
		f = 'Data\\Results\\YelpNYC\\Product Behavioral\\'+cls[i]+'\\X_bal_sets.sav'
		...
		k = 1
		for mm in result:
			filename2 = 'Data\\Results\\YelpNYC\\Product Behavioral\\'+cls[i]+'\\Model_'+str(k)+'.sav'			# Change the path according to the file location
