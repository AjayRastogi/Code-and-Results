...
if __name__ == "__main__":
	...
	cls = ["SVM", "LR", "MLP"]		
	X  = pd.read_excel('C:\\Users\\Ajay Rastogi\\Desktop\\Extended Version (Journal)\\PeerJ\\PeerJ-Paper\\Data\\YelpNYC\\Resulting Features\\Review_Centric_Behavioral_Features.xlsx', sheetname=0)		# Change the path according to the file location
	X['RL(r)'] =(X['RL(r)']-X['RL(r)'].min())/(X['RL(r)'].max()-X['RL(r)'].min())										# Min-max normalization on RL
	X1 = pd.read_excel('C:\\Users\\Ajay Rastogi\\Desktop\\Extended Version (Journal)\\PeerJ\\PeerJ-Paper\\Data\\YelpNYC\\Resulting Features\\Review_Centric_Textual_Features.xlsx', sheetname=0)			# Change the path according to the file location
	#print(X.head())
	#print(X.shape)
	#print(X1.head())
	#print(X1.shape)
	Y= pd.read_excel('C:\\Users\\Ajay Rastogi\\Desktop\\Extended Version (Journal)\\PeerJ\\PeerJ-Paper\\Data\\YelpNYC\\Labels_for_reviews.xlsx', sheetname=0)			# Change the path according to the file location
	Y = Y.iloc[:, 0:1]
	...
	"""for i in range(0,3):
		...
		filename = 'Data\\Results\\YelpNYC\\Review Hybrid\\'+cls[i]+'\\FPR_TPR_Threshold_Review_Hybrid.xlsx'		# File for Writing FPR and TPR values at different thresholds for ROC curve analysis
		filename1 = 'Data\\Results\\YelpNYC\\Review Hybrid\\'+cls[i]+'\\Pre_Recall_Threshold_Review_Hybrid.xlsx'	# File for Writing Precision and Recall values at different thresholds for Precision-recall curve analysis
		f = 'Data\\Results\\YelpNYC\\Review Hybrid\\'+cls[i]+'\\X_bal_sets.sav'
		...
		k = 1
		for mm in result:
			filename2 = 'Data\\Results\\YelpNYC\\Review Hybrid\\'+cls[i]+'\\Model_'+str(k)+'.sav'			# Change the path according to the file location
