...
if __name__ == "__main__":
	...
	cls = ["SVM", "LR", "MLP"]		
	X  = pd.read_excel('C:\\Users\\Ajay Rastogi\\Desktop\\Extended Version (Journal)\\PeerJ\\PeerJ-Paper\\Data\\YelpNYC\\Resulting Features\\Reviewer_Centric_Behavioral_Features.xlsx', sheetname=0)		# Change the path according to the file location
	X['MNR(u)'] =(X['MNR(u)']-X['MNR(u)'].min())/(X['MNR(u)'].max()-X['MNR(u)'].min())									# Min-max normalization on MNR
	X1 = pd.read_excel('C:\\Users\\Ajay Rastogi\\Desktop\\Extended Version (Journal)\\PeerJ\\PeerJ-Paper\\Data\\YelpNYC\\Resulting Features\\Reviewer_Centric_Textual_Features.xlsx', sheetname=0)		# Change the path according to the file location
	X1['ARL(u)'] =(X1['ARL(u)']-X1['ARL(u)'].min())/(X1['ARL(u)'].max()-X1['ARL(u)'].min())								# Min-max normalization on ARL
	#print(X.head())
	#print(X.shape)
	#print(X1.head())
	#print(X1.shape)
	Y= pd.read_excel('C:\\Users\\Ajay Rastogi\\Desktop\\Extended Version (Journal)\\PeerJ\\PeerJ-Paper\\Data\\YelpNYC\\Labels_for_reviewers.xlsx', sheetname=0)			# Change the path according to the file location
	Y = Y.iloc[:, 0:1]
	...
	"""for i in range(0,3):
		...
		filename = 'Data\\Results\\YelpNYC\\Reviewer Hybrid\\'+cls[i]+'\\FPR_TPR_Threshold_Reviewer_Hybrid.xlsx'		# File for Writing FPR and TPR values at different thresholds for ROC curve analysis
		filename1 = 'Data\\Results\\YelpNYC\\Reviewer Hybrid\\'+cls[i]+'\\Pre_Recall_Threshold_Reviewer_Hybrid.xlsx'	# File for Writing Precision and Recall values at different thresholds for Precision-recall curve analysis
		f = 'Data\\Results\\YelpNYC\\Reviewer Hybrid\\'+cls[i]+'\\X_bal_sets.sav'
		...
		k = 1
		for mm in result:
			filename2 = 'Data\\Results\\YelpNYC\\Reviewer Hybrid\\'+cls[i]+'\\Model_'+str(k)+'.sav'			# Change the path according to the file location
