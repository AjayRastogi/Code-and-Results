...
if __name__ == "__main__":
	...
	cls = ["SVM", "LR", "MLP"]		
	X  = pd.read_excel('Data\\YelpNYC\\Resulting Features\\Reviewer_Centric_Behavioral_Features.xlsx', sheetname=0)		# Change the path according to the file location
	X['MNR(u)'] =(X['MNR(u)']-X['MNR(u)'].min())/(X['MNR(u)'].max()-X['MNR(u)'].min())									# Min-max normalization on MNR
	X1 = pd.read_excel('Data\\YelpNYC\\Resulting Features\\Reviewer_Centric_Textual_Features.xlsx', sheetname=0)		# Change the path according to the file location
	X1['ARL(u)'] =(X1['ARL(u)']-X1['ARL(u)'].min())/(X1['ARL(u)'].max()-X1['ARL(u)'].min())								# Min-max normalization on ARL
	#print(X.head())
	#print(X.shape)
	#print(X1.head())
	#print(X1.shape)
	Y= pd.read_excel('Data\\YelpNYC\\Labels_for_reviewers.xlsx', sheetname=0)			# Change the path according to the file location
	Y = Y.iloc[:, 0:1]
