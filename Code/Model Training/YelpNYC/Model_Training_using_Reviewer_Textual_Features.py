...
if __name__ == "__main__":
	...
	cls = ["SVM", "LR", "MLP"]			
	X = pd.read_excel('Data\\YelpNYC\\Resulting Features\\Reviewer_Centric_Textual_Features.xlsx', sheetname=0)			# Change the path according to the file location
	X['ARL(u)'] =(X['ARL(u)']-X['ARL(u)'].min())/(X['ARL(u)'].max()-X['ARL(u)'].min())									# Min-max normalization on ARL
	#print(X.head())
	#print(X.shape)
	...
	for i in range(0,3):
		...
		filename = 'Data\\Results\\YelpNYC\\Reviewer Textual\\'+cls[i]+'\\FPR_TPR_Threshold_Reviewer_Textual.xlsx'		# File for Writing FPR and TPR values at different thresholds for ROC curve analysis
		filename1 = 'Data\\Results\\YelpNYC\\Reviewer Textual\\'+cls[i]+'\\Pre_Recall_Threshold_Reviewer_Textual.xlsx'	# File for Writing Precision and Recall values at different thresholds for Precision-recall curve analysis
		f = 'Data\\Results\\YelpNYC\\Reviewer Textual\\'+cls[i]+'\\X_bal_sets.sav'
		...
		k = 1
		for mm in result:
			filename2 = 'Data\\Results\\YelpNYC\\Reviewer Textual\\'+cls[i]+'\\Model_'+str(k)+'.sav'			# Change the path according to the file location
