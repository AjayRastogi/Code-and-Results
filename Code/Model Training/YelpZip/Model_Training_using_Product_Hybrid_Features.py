...
if __name__ == "__main__":
	...
	cls = ["SVM", "LR", "MLP"]		
	X  = pd.read_excel('Data\\YelpZip\\Resulting Features\\Product_Centric_Behavioral_Features.xlsx', sheetname=0)		# Change the path according to the file location
	X['MNR(p)'] =(X['MNR(p)']-X['MNR(p)'].min())/(X['MNR(p)'].max()-X['MNR(p)'].min())									# Min-max normalization on MNR
	X1 = pd.read_excel('Data\\YelpZip\\Resulting Features\\Product_Centric_Textual_Features.xlsx', sheetname=0)			# Change the path according to the file location
	X1['ARL(p)'] = (X1['ARL(p)']-X1['ARL(p)'].min())/(X1['ARL(p)'].max()-X1['ARL(p)'].min())									# Min-max normalization on ARL
	#print(X.head())
	#print(X.shape)
	#print(X1.head())
	#print(X1.shape)
	Y= pd.read_excel('Data\\YelpZip\\Labels_for_products.xlsx', sheetname=0)			# Change the path according to the file location
	Y = Y.iloc[:, 0:1]
	...
	for i in range(0,3):
		...
		filename = 'Data\\Results\\YelpZip\\Product Hybrid\\'+cls[i]+'\\FPR_TPR_Threshold_Product_Hybrid.xlsx'		# File for Writing FPR and TPR values at different thresholds for ROC curve analysis
		filename1 = 'Data\\Results\\YelpZip\\Product Hybrid\\'+cls[i]+'\\Pre_Recall_Threshold_Product_Hybrid.xlsx'	# File for Writing Precision and Recall values at different thresholds for Precision-recall curve analysis
		f = 'Data\\Results\\YelpZip\\Product Hybrid\\'+cls[i]+'\\X_bal_sets.sav'
		...
		k = 1
		for mm in result:
			filename2 = 'Data\\Results\\YelpZip\\Product Hybrid\\'+cls[i]+'\\Model_'+str(k)+'.sav'			# Change the path according to the file location
