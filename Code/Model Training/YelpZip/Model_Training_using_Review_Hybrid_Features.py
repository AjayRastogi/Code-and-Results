...
if __name__ == "__main__":
	...
	cls = ["SVM", "LR", "MLP"]		
	X  = pd.read_excel('Data\\YelpZip\\Resulting Features\\Review_Centric_Behavioral_Features.xlsx', sheetname=0)		# Change the path according to the file location
	X['RL(r)'] =(X['RL(r)']-X['RL(r)'].min())/(X['RL(r)'].max()-X['RL(r)'].min())										# Min-max normalization on RL
	X1 = pd.read_excel('Data\\YelpZip\\Resulting Features\\Review_Centric_Textual_Features.xlsx', sheetname=0)			# Change the path according to the file location
	#print(X.head())
	#print(X.shape)
	#print(X1.head())
	#print(X1.shape)
	Y= pd.read_excel('Data\\YelpZip\\Labels_for_reviews.xlsx', sheetname=0)				# Change the path according to the file location
	Y = Y.iloc[:, 0:1]
