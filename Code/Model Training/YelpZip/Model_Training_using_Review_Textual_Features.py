...
if __name__ == "__main__":
	...
	cls = ["SVM", "LR", "MLP"]			
	X = pd.read_excel('Data\\YelpZip\\Resulting Features\\Review_Centric_Textual_Features.xlsx', sheetname=0)		# Change the path according to the file location
	#print(X.head())
	#print(X.shape)
	...
	for i in range(0,3):
		...
		filename = 'Data\\Results\\YelpZip\\Review Textual\\'+cls[i]+'\\FPR_TPR_Threshold_Review_Textual.xlsx'		# File for Writing FPR and TPR values at different thresholds for ROC curve analysis
		filename1 = 'Data\\Results\\YelpZip\\Review Textual\\'+cls[i]+'\\Pre_Recall_Threshold_Review_Textual.xlsx'	# File for Writing Precision and Recall values at different thresholds for Precision-recall curve analysis
		f = 'Data\\Results\\YelpZip\\Review Textual\\'+cls[i]+'\\X_bal_sets.sav'
		...
		k = 1
		for mm in result:
			filename2 = 'Data\\Results\\YelpZip\\Review Textual\\'+cls[i]+'\\Model_'+str(k)+'.sav'			
